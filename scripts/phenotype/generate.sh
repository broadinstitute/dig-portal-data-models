...
uv run python scripts/phenotype/01_parse_sources.py
echo ""
...
if [ "$SKIP_OWL" = true ] && [ -f "$XREF_FILE" ]; then
    ...
    uv run python scripts/phenotype/02_parse_efo_xrefs.py
fi
echo ""
...
if [ "$SKIP_API" = true ]; then
    ...
    uv run python scripts/phenotype/03_enrich.py --skip-api
else
    echo ">>> Step 3: Enriching mappings (OLS API + GWAS Catalog + labels)..."
    uv run python scripts/phenotype/03_enrich.py
fi
echo ""

# Step 4: Generate versioned output
echo ">>> Step 4: Generating v${VERSION} output..."
uv run python scripts/phenotype/04_generate_output.py --version "$VERSION"
echo ""

# Step 5: Quality report
echo ">>> Step 5: Quality report..."
uv run python scripts/phenotype/05_quality_report.py --version "$VERSION"
echo ""
